...
- Icons live in `artwork/` (SVG + PNG sources, both light/dark variants for the app icon and tray icon) — regenerate `assets/branding/`, `assets/tray/`, `macos/Runner/Assets.xcassets/AppIcon.appiconset/`, `windows/runner/resources/app_icon.ico` from there if the mark changes. macOS's `AppIcon.appiconset/Contents.json` uses the `"appearances":[{"appearance":"luminosity","value":"dark"}]` key per image entry for the dark variant, macOS 14+ (Xcode 15+ asset-catalog feature, distinct from — and much simpler than — Xcode 26's newer multi-layer Icon Composer/Liquid Glass format, which this project does not use). The macOS tray icon stays a single `isIconTemplate` asset (OS auto-recolors it); Windows/Linux tray icons don't get OS recoloring, so `TrayService` picks the light/dark-glyph asset itself from `PlatformDispatcher.platformBrightness`.
